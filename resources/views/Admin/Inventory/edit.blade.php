                            ...
                            <form action="{{ route('inventory.update', $inventoryLog->id) }}" method="POST"
                                        ...
                                        <select name="category_id" id="category_id" class="form-select" required>
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ $inventoryLog->product->category_id == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="variation_id" id="variation_id" class="form-select" required>
                                            ...
                                            @if ($inventoryLog->variation)
                                                <option value="{{ $inventoryLog->variation->id }}" selected>
                                                    Màu:
                                                    {{ $inventoryLog->variation->color ? $inventoryLog->variation->color->color : 'Chưa xác định' }},
                                                    Kích thước:
                                                    {{ $inventoryLog->variation->size ? $inventoryLog->variation->size->size : 'Chưa xác định' }},
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="quantity_change"
                                        value="{{ $inventoryLog->quantity_change }}" class="form-control" required>
                                </div>

                                <div class="mb-3">
                                    <label for="note" class="form-label">Ghi Chú</label>
                                    <input type="text" name="note" value="{{ $inventoryLog->note }}"
