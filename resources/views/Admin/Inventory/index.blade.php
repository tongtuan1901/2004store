                                        ...
                                        <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4"
                                            ...
                                            <ul>
                                                @foreach ($lowStockItems as $item)
                                                    <li>
                                                        <strong>{{ $item['product_name'] }}</strong> - Size:
                                                        {{ $item['size'] }}, Màu sắc: {{ $item['color'] }}, Số lượng:
                                                        {{ $item['quantity'] }}, Danh mục: {{ $item['category'] }}
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </div>
                ...
                <a href="{{ route('inventory.create') }}" class="inline-flex items-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                    Thêm Bản Ghi Tồn Kho
                                    ...
                                    <form action="{{ route('inventory.index') }}" method="GET" class="mb-4">
                                        ...
                                        <div class="flex mb-4">
                                            <div class="mr-4">
                                                <label for="start_date" class="block text-sm font-medium">Ngày Bắt
                                                    Đầu</label>
                                                <input type="date" id="start_date" name="start_date"
                                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                            <div>
                                                <label for="end_date" class="block text-sm font-medium">Ngày Kết
                                                    Thúc</label>
                                            ...
                                            <div class="ml-4 flex items-end">
                                                <button type="submit"
                                                    class="inline-flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">Lọc</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="mb-4">
                                <div class=" mg-top-40 sherah-dflex sherah-dflex-gap-30 justify-content-end"> <button> <a
                                            href="{{ route('inventory.create') }}"
                                            class="inline-flex items-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                                            Thêm Bản Ghi Tồn Kho
                                        </a> </button> </div>
                            </div>
                                    ...
                                    <table id="sherah-table__vendor" class="sherah-table__main sherah-table__main-v3">
                                        ...
                                        <tbody class="sherah-table__body">
                                            @foreach ($inventoryLogs as $index => $log)
                                                <tr>
                                                    ...
                                                    <td class="sherah-table__column-5 sherah-table__data-5">
                                                        <div class="sherah-table__product-content">
                                                            <p class="sherah-table__product-desc">
                                                                @if ($log->variation)
                                                                    Size:
                                                                    {{ $log->variation->size ? $log->variation->size->size : 'N/A' }},
                                                                    Màu sắc:
                                                                    {{ $log->variation->color ? $log->variation->color->color : 'N/A' }},
                                                    ...
                                                    <td class="sherah-table__column-4 sherah-table__data-4">
                                                        <div class="sherah-table__product-content">
                                                            <p class="sherah-table__product-desc">
                                                                {{ $log->quantity_change }}</p>
                                                        </div>
                                                    </td>
                                                    <td class="sherah-table__column-4 sherah-table__data-4">
                                                        <div class="sherah-table__product-content">
                                                            <p class="sherah-table__product-desc">
                                                                {{ $log->note ?? 'Không có ghi chú' }}</p>
                                                        </div>
                                                    </td>
                                            ...
                                            @if ($inventoryLogs->isEmpty())
                                                <tr>
                                                    <td colspan="8" class="py-2 px-4 text-center text-gray-500">Không
                                                        có bản ghi tồn kho nào.</td>
                                                </tr>
                                            @endif
                                        </tbody>
                                    </table>
