    ...
    <script>
        const ctx = document.getElementById('revenueChart').getContext('2d');
         const revenueData = @json($revenueData); // Lấy dữ liệu doanh thu từ controller
    console.log(revenueData); // Dữ liệu doanh thu

        const revenueChart = new Chart(ctx, {
            type: 'line', // Hoặc 'bar' nếu bạn muốn
            data: {
                labels: Array.from({length: revenueData.length}, (_, i) => i + 1), // Ngày 1 đến 31
                datasets: [{
                    label: 'Doanh thu',
                    data: revenueData,
